...
const root = path.resolve(args.root ?? 'training');
const cacheRoot = path.resolve(args.cacheRoot ?? process.env.WEATHERWORLD_TRAINING_CACHE ?? path.join(process.env.USERPROFILE ?? process.env.HOME ?? '.', 'WeatherWorldTrainingCache'));
const rawSpcRoot = path.join(cacheRoot, 'spc');
...
await mkdir(rawSpcRoot, { recursive: true });
await mkdir(path.join(root, 'targets', 'spc'), { recursive: true });

const jobs = dates.map(date => {
  const compact = date.replaceAll('-', '');
  const rawRoot = path.join(rawSpcRoot, compact);
  return { date, rawRoot, manifest: path.join(rawRoot, 'manifest.json'), fetch: null, normalize: null };
});

console.log('Training target population 2.36.0');
...
await runPool(normalizable, normalizeConcurrency, async job => {
  job.normalize = await runNode([
    ...
    '--output', path.join(root, 'targets', 'spc')
...
const build = await runNode([
  args.pipelineScript ?? 'scripts/validate-training-targets.mjs', '--root', root,
  '--input', path.join(root, 'targets', 'spc'), '--concurrency', String(pipelineConcurrency),
  '--progress', '--write-reports'
], { quiet: false });
if (build.code !== 0) printChildFailure('Training validation', 'target validation', build);
...
const reportPath = path.join(root, 'catalog', 'population-report.json');
...
async function resolveDates(options) {
  if (options.retryFailed) {
    const reportPath = path.join(path.resolve(options.root ?? 'training'), 'catalog', 'population-report.json');
  ...
  if (!options.start) throw new TypeError('Usage: npm run training:populate-targets -- --dates YYYY-MM-DD,... | --start YYYY-MM-DD [--end YYYY-MM-DD]');
